...
def show(sgroup_url=None, troop_url=None, key_url=None):
    ...
    if key_url == "delete":
        if troop is None:
            return "", 404
        if not lockedTroop and request.form and "confirm" in request.form:
    ...
    if request.method == "GET" and request.args and "action" in request.args:
        ...
        if action == "lookupperson":
            ...
            if troop is None or key_url is None or lockedTroop:
    ...
    if request.method == "POST" and request.form and "action" in request.form:
        ...
        if action == "saveattendance":
            if troop is None or scoutgroup is None or key_url is None or lockedTroop:
